...
//закрытие отчета по кнопке и мимо модального окна
...
//открыть отчет + анимация открытия
const openReport = () => {
...
export const reportControl = () => {
  reportTable.addEventListener('click', async ({ target }) => {
    ...
    if (targetDel) {
      ...
      const title = confirm('Вы действительно хотите удалить строку?');
      reportRow.remove();
      financeConrtol();
      clearChart();
    }
  ...
  //прелоудер при нажатии на кнопку отчет
